<script lang="ts">
	...
	import { Alert, Header, RemoveButton } from '$lib/components';
...
<main class="min-h-screen">
	...
	<section class="flex flex-wrap justify-center p-8 sm:p-16 gap-8">
		<button class="btn" on:click={() => triggerCrawl(fetch)}>Trigger Crawl</button>
		<button class="btn" on:click={async () => (data.feeds = await importFeeds(fetch))}>
			Import Feeds
		</button>
	</section>
			...
			<ul>
				{#each data.feeds as feed (feed.key)}
					<li>
						...
						<form method="POST" action="?/removeFeed" class="inline">
			...
			<ul>
				{#each data.keywords as keyword (keyword.key)}
					<li>
						...
						<form method="POST" action="?/removeKeyword" class="inline ml-2">
							<input type="hidden" name="key" value={keyword.key} />
							<RemoveButton />
						</form>
						<small class="block">

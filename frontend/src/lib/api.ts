...
export async function deleteFeed(fetch: Fetch, url: string): Promise<void> {
	await fetch(`${env.PUBLIC_BACKEND_URL}/feeds`, {
		method: 'DELETE',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ key: url })
	});
...
export async function importFeeds(fetch: Fetch): Promise<Feed[]> {
	const res = await fetch(`${env.PUBLIC_BACKEND_URL}/feeds/import`, { cache: 'no-store' });
...
export async function createKeyword(fetch: Fetch, value: string): Promise<Keyword> {
...
export async function deleteKeyword(fetch: Fetch, key: string): Promise<void> {
	await fetch(`${env.PUBLIC_BACKEND_URL}/keywords/${key}`, { method: 'DELETE' });
}

// crawler
export async function triggerCrawl(fetch: Fetch): Promise<void> {
	await fetch(`${env.PUBLIC_BACKEND_URL}/__space/v0/actions`, {
		method: 'POST',

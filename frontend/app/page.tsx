...
async function getFeeds() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/feeds`, { next: { revalidate: 1 } });
  if (!res.ok) return [];
  return res.json();
}
        ...
        <div className='bg-primary opacity-95 absolute p-8 sm:p-16 md:p-24 shadow-2xl rounded-3xl w-full lg:w-2/3 max-w-[900px] border-solid border-2 border-primary'>
          <div className='my-10'>
            ...
            <p className='text-xl text-slate-400 sm:text-2xl md:text-3xl my-4 md:my-6'>
              An AI-powered Slack alert bot to <span className='underline'>subscribe</span>, <span className='underline'>classify</span> and <span className='underline'>notify</span> for keywords on the web.
            </p>
          </div>
